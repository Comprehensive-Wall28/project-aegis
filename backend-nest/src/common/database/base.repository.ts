import { Document, Model, UpdateQuery, ClientSession, MongooseBulkWriteOptions, FilterQuery } from 'mongoose';
...
import { RepositoryError, RepositoryErrorCode } from './repository.error';
...
export abstract class BaseRepository<T extends Document> {
     ...
     */
    async findOne(filter: SafeFilter<T>, options: QueryOptions = {}): Promise<T | null> {
        const sanitizedFilter = QuerySanitizer.sanitizeFilter(filter);

        try {
            let query = this.model.findOne(sanitizedFilter as FilterQuery<T>);
            query = this.applyOptions(query, options);
     ...
     */
    async findMany(filter: SafeFilter<T>, options: QueryOptions = {}): Promise<T[]> {
        const sanitizedFilter = QuerySanitizer.sanitizeFilter(filter);

        try {
            let query = this.model.find(sanitizedFilter as FilterQuery<T>);
            query = this.applyOptions(query, options);
    ...
    async findPaginated(
        ...
        const sanitizedFilter = QuerySanitizer.sanitizeFilter(filter) as any;
    ...
    async updateById(
        ...
        try {
            const result = await this.model.findByIdAndUpdate(
                sanitizedId,
                sanitizedUpdate as UpdateQuery<T>,
    ...
    async updateOne(
        ...
        try {
            const result = await this.model.findOneAndUpdate(
                sanitizedFilter as FilterQuery<T>,
                sanitizedUpdate as UpdateQuery<T>,
     ...
     */
    async updateMany(filter: SafeFilter<T>, data: UpdateQuery<T>): Promise<number> {
        const sanitizedFilter = QuerySanitizer.sanitizeFilter(filter);
        const sanitizedUpdate = QuerySanitizer.sanitizeUpdate(data);
    ...
    async deleteOne(filter: SafeFilter<T>): Promise<boolean> {
        ...
        try {
            const result = await this.model.findOneAndDelete(sanitizedFilter as FilterQuery<T>).exec();
            return result !== null;
        } catch (error) {
    ...
    async deleteMany(filter: SafeFilter<T>): Promise<number> {
        ...
        try {
            const result = await this.model.deleteMany(sanitizedFilter as FilterQuery<T>).exec();
    ...
    async bulkWrite(operations: BulkWriteOperation<T>[], options: MongooseBulkWriteOptions = {}): Promise<BulkWriteResult> {
        ...
        const sanitizedOps = operations.map(op => {
            ...
            if ('updateOne' in op) {
                return {
                    updateOne: {
                        filter: QuerySanitizer.sanitizeFilter(op.updateOne.filter),
            ...
            if ('updateMany' in op) {
                return {
                    updateMany: {
                        filter: QuerySanitizer.sanitizeFilter(op.updateMany.filter),
            ...
            if ('deleteOne' in op) {
                return {
                    deleteOne: {
                        filter: QuerySanitizer.sanitizeFilter(op.deleteOne.filter)
                    }
                };
            }
            if ('deleteMany' in op) {
                return {
                    deleteMany: {
                        filter: QuerySanitizer.sanitizeFilter(op.deleteMany.filter)
                    }
                };
            }
            return op;
    ...
    async aggregate<R>(pipeline: object[]): Promise<R[]> {
        ...
        const sanitizedPipeline = pipeline.map(stage => {
            ...
            for (const [key, value] of Object.entries(stageObj)) {
                if (key === '$match') {
                    result[key] = QuerySanitizer.sanitizeFilter(value);
                } else if (key === '$sort') {
    ...
    async count(filter: SafeFilter<T> = {}): Promise<number> {
        ...
        try {
            return await this.model.countDocuments(sanitizedFilter as FilterQuery<T>).exec();
        } catch (error) {
    ...
    async exists(filter: SafeFilter<T>): Promise<boolean> {
        ...
        try {
            const result = await this.model.exists(sanitizedFilter as FilterQuery<T>).exec();
            return result !== null;
        } catch (error) {
    ...
    private applyOptions(query: any, options: QueryOptions): any {
        ...
        if (options.select) {
            const sanitizedSelect = typeof options.select === 'string'
