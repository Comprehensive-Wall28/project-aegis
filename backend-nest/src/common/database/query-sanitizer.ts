...
export class QuerySanitizer {
     ...
     * (Renamed from sanitizeQuery to align with new interface)
     */
    static sanitizeFilter<T>(filter: unknown): Record<string, unknown> {
        ...
        if (Array.isArray(filter)) {
            return filter.map((item) => this.sanitizeFilter(item)) as unknown as Record<string, unknown>;
        }

        const sanitized: Record<string, unknown> = {};
        const queryObj = filter as Record<string, unknown>;
        ...
        for (const [key, value] of Object.entries(queryObj)) {
            ...
            if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                ...
                if (key === '$and' || key === '$or') {
                    sanitized[key] = value.map((item) => this.sanitizeFilter(item));
