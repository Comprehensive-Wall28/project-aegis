...
// import { Request } from 'express'; // Removed to support Fastify
  ...
  async logFailedAuth(
    ...
    try {
      ...
      await AuditLog.create({
        ...
        metadata: {
          ...metadata,
          attemptedIdentifier: identifier.substring(0, 3) + '***',
        },
        timestamp,
      });

      this.logger.warn(
        `Auth failure: ${action} attempt for ${identifier} from ${ipAddress}`,
      );
    } catch (error) {
