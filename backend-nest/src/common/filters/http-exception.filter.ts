...
import { AuditAction, AuditStatus } from '../../modules/audit/schemas/audit-log.schema';
...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    async catch(exception: unknown, host: ArgumentsHost) {
        ...
        if (status >= 500) {
            this.logger.error(`Http Exception: ${message}`, (exception as Error).stack);
        ...
        if ((exception as any).code === 'EBADCSRFTOKEN') {
            response.status(403).send({ code: 'EBADCSRFTOKEN', message: 'Invalid CSRF token' });
            return;
        }
