...
export class AuthService {
  ...
  async register(registerDto: RegisterDto, req?: Request): Promise<any> {
    try {
      ...
      if (existingByEmail || existingByUsername) {
        this.logger.warn(
          `Failed registration: Email ${email} or username ${username} exists`,
        );
  ...
  async login(
    ...
    try {
      ...
      if (!user || !user.passwordHash) {
        this.logger.warn(
          `Failed login for email: ${normalizedEmail} (user not found)`,
        );
      ...
      if (!verified) {
        this.logger.warn(
          `Failed login for email: ${normalizedEmail} (invalid password)`,
        );
        await this.auditService.logFailedAuth(
          normalizedEmail,
          'LOGIN_FAILED',
      ...
      if (user.webauthnCredentials && user.webauthnCredentials.length > 0) {
        ...
        await this.usersRepository.updateChallenge(
          user._id.toString(),
          options.challenge,
        );
  ...
  async discoverUser(
    email: string,
  ): Promise<{ username: string; pqcPublicKey: string }> {
    try {
      if (!email) {
  ...
  async removePassword(userId: string, req?: Request): Promise<void> {
    try {
      ...
      if (user.webauthnCredentials.length === 0) {
        throw new BadRequestException(
          'Must have at least one passkey registered to remove password',
        );
      }
  ...
  async removePasskey(
    ...
    try {
      ...
      const credentialExists = user.webauthnCredentials.some(
