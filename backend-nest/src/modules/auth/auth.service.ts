...
export class AuthService {
  ...
  async register(
    ...
    try {
      ...
      if (existingByEmail || existingByUsername) {
        this.logger.warn(
          `Failed registration: Email ${data.email} or username ${data.username} exists`,
        );
  ...
  ): Promise<UserResponseDto | { status: '2FA_REQUIRED'; options: any }> {
    try {
      ...
      if (user.webauthnCredentials && user.webauthnCredentials.length > 0) {
        ...
        await this.userRepository.updateChallenge(
          user._id.toString(),
          options.challenge,
        );
  ...
  async updateProfile(
    ...
    try {
      ...
      if (data.username) {
        const usernameTaken = await this.userRepository.isUsernameTaken(
          data.username,
          userId,
        );
        if (usernameTaken) {
