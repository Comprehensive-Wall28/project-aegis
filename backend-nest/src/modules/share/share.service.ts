...
export class ShareService {
    ...
    async inviteToFile(userId: string, data: InviteDto, req?: any): Promise<SharedFileDocument> {
        if (!data.fileId || !data.email || !data.encryptedSharedKey) {
            throw new BadRequestException('Missing required fields');
        }

        const file = await this.vaultRepository.findByIdAndOwner(data.fileId, userId);
        if (!file) {
        ...
        const existing = await this.sharedFileRepository.findByFileAndUser(data.fileId, recipient._id.toString());
        if (existing) {
        ...
        await this.auditService.logAuditEvent(userId, 'FILE_SHARE', 'SUCCESS', req, {
            fileId: data.fileId,
    ...
    async getSharedFileKey(userId: string, fileId: string): Promise<{ encryptedSharedKey: string }> {
        const sharedFile = await this.sharedFileRepository.findByFileAndUser(fileId, userId);
        if (!sharedFile) {
    ...
    async createLink(userId: string, data: CreateLinkDto, req?: any): Promise<SharedLinkDocument> {
        if (!data.resourceId || !data.resourceType || !data.encryptedKey) {
            throw new BadRequestException('Missing required fields');
        }

        if (data.resourceType === 'file') {
            const file = await this.vaultRepository.findByIdAndOwner(data.resourceId, userId);
            if (!file) throw new NotFoundException('File not found');
        } else {
            throw new BadRequestException('Invalid resource type. Folder sharing is disabled.');
        }
        ...
        await this.auditService.logAuditEvent(userId, 'LINK_SHARE_CREATE', 'SUCCESS', req, {
    ...
    async getMyLinks(userId: string, page: number = 1, limit: number = 5): Promise<any> {
        ...
        const { links, total } = await this.sharedLinkRepository.findLinksByCreator(userId, skip, limit);
    ...
    async revokeLink(userId: string, linkId: string, req?: any): Promise<void> {
        ...
        await this.auditService.logAuditEvent(userId, 'LINK_SHARE_REVOKE', 'SUCCESS', req, {
            linkId
        });
    }
}
