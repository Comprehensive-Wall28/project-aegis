...
describe('NoteFolderService (Integration)', () => {
  ...
  it('should not allow duplicate names in the same parent', async () => {
    ...
    await expect(
      folderService.create(userId.toHexString(), dto),
  ...
  it('should remove a folder and its descendants', async () => {
    const root = await folderService.create(userId.toHexString(), {
      name: 'Root',
    });
    ...
    await expect(
      folderService.findOne(rootId, userId.toHexString()),
    ).rejects.toThrow('Folder not found');
    await expect(
      folderService.findOne(nestedId, userId.toHexString()),
    ).rejects.toThrow('Folder not found');
  });
});
