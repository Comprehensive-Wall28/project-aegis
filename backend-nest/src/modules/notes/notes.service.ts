...
export class NotesService {
  ...
  async create(
    ...
    const note = await this.noteRepository.create({
      ...
      noteFolderId: createDto.noteFolderId
        ? new Types.ObjectId(createDto.noteFolderId)
        : undefined,
  ...
  async getContent(
    id: string,
    userId: string,
  ): Promise<{ buffer: Buffer; note: Note }> {
  ...
  async getContentStream(
    id: string,
    userId: string,
  ): Promise<{ stream: Readable; note: Note }> {
    const note = await this.findOne(id, userId);
  ...
  async updateMetadata(
    id: string,
    userId: string,
    updateDto: UpdateNoteMetadataDTO,
    req?: Request,
  ): Promise<Note> {
  ...
  async updateContent(
    id: string,
    userId: string,
    updateDto: UpdateNoteContentDTO,
    req?: Request,
  ): Promise<Note> {
    const note = await this.findOne(id, userId);
    ...
    const updated = await this.noteRepository.updateOne(
      { _id: id, userId },
      {
        ...
        ...(updateDto.encryptedTitle
          ? { encryptedTitle: updateDto.encryptedTitle }
          : {}),
      },
      { returnNew: true },
    );
    ...
    await this.gridFsService
      ...
      .catch((err) =>
        this.logger.warn(
          `Failed to cleanup old GridFS file ${oldFileId}: ${err.message}`,
        ),
      );
  ...
  async remove(id: string, userId: string, req?: Request): Promise<void> {
    ...
    // Cleanup GridFS
    await this.gridFsService
      .deleteFile(note.gridFsFileId)
      .catch((err) =>
        this.logger.warn(
          `Failed to delete GridFS file ${note.gridFsFileId}: ${err.message}`,
        ),
      );
