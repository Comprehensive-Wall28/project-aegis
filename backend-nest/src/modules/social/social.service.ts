...
export class SocialService extends BaseService<RoomDocument, SocialRepository> {
  ...
  async joinRoom(
    ...
    await this.socialRepository.addMember(
      room._id.toString(),
      userId,
      'member',
      encryptedRoomKey,
  ...
  async deleteCollection(
    ...
    const room = await this.socialRepository.findById(
  ...
  async reorderCollections(
    ...
    for (let i = 0; i < collectionIds.length; i++) {
      await this.collectionRepository.updateById(collectionIds[i], {
        $set: { order: i },
      } as any);
    }
  ...
  async getRoomContent(
    ...
    this.logger.log(
      `[Performance] getRoomContent for room ${roomId} took ${duration}ms`,
    );
