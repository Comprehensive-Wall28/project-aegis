...
export class ScraperService implements OnModuleDestroy {
    ...
    private readonly LOCALES = ['en-US', 'en-GB', 'en-CA', 'fr-FR', 'de-DE', 'es-ES'];
    private readonly TIMEZONES = ['America/New_York', 'Europe/London', 'Europe/Paris', 'Europe/Berlin', 'Asia/Tokyo'];
    ...
    private setupQueueListeners() {
        this.scrapeQueue.on('active', () => {
            this.logger.debug(`[ScrapeQueue] Task started. Queue size: ${this.scrapeQueue.size}, Pending: ${this.scrapeQueue.pending}`);
        });
    ...
    private loadReadability() {
        try {
            ...
            const readabilityPath = path.resolve(process.cwd(), 'node_modules/@mozilla/readability/Readability.js');
    ...
    private async getBrowser(): Promise<Browser> {
        ...
        if (this.browserInstance && this.advancedScrapeCount >= this.MAX_ADVANCED_SCRAPES) {
            if (this.scrapeQueue.pending === 0) {
                this.logger.log(`Request limit (${this.MAX_ADVANCED_SCRAPES}) reached. Recycling browser...`);
                await this.closeBrowser();
            }
        }
        ...
        this.browserLaunchPromise = chromium.launch(launchOptions).then((browser: Browser) => {
    ...
    private isBlockedDomain(url: string): boolean {
        return this.BLOCKED_DOMAIN_PATTERNS.some(pattern => url.includes(pattern));
    }

    async simpleScrape(targetUrl: string): Promise<{ data: ScrapeResult | null, reason?: string }> {
        try {
            ...
            if (!result.success) return { data: null, reason: 'OGS returned false success' };
    ...
    async smartScrape(targetUrl: string): Promise<ScrapeResult> {
        ...
        this.logger.log(`Simple Scrape failed (${reason}). Switching to Advanced Scrape.`);
        return this.advancedScrape(targetUrl);
    }

    async advancedScrape(targetUrl: string, retryCount = 0): Promise<ScrapeResult> {
        const MAX_RETRIES = 2;
        try {
            const result = await this.scrapeQueue.add(() => this.advancedScrapeInternal(targetUrl), {
                timeout: 60000
            });

            if (result?.scrapeStatus === 'blocked' && retryCount < MAX_RETRIES) {
                const delay = (retryCount + 1) * 5000;
                this.logger.warn(`Blocked. Retrying in ${delay}ms... (Attempt ${retryCount + 1}/${MAX_RETRIES})`);
            ...
            this.logger.error(`Advanced scrape failed for ${targetUrl}: ${error.message}`);
            return { title: '', description: '', image: '', favicon: '', scrapeStatus: 'failed' };
        }
    }

    private async advancedScrapeInternal(targetUrl: string): Promise<ScrapeResult> {
        ...
        try {
            ...
            await page.route('**/*', (route) => {
                ...
                if (['media', 'font', 'stylesheet'].includes(type) || this.isBlockedDomain(url)) {
                    route.abort();
                } else if (type === 'image') {
            ...
            const metadata = await this.metascraper({ html: pageData.html, url: pageData.url });
            return {
    ...
    async readerScrape(targetUrl: string): Promise<ReaderContentResult> {
        return this.scrapeQueue.add(() => this.readerScrapeInternal(targetUrl), {
            timeout: 90000
        }) as Promise<ReaderContentResult>;
    }

    private async readerScrapeInternal(targetUrl: string): Promise<ReaderContentResult> {
        this.advancedScrapeCount++;
        let context: BrowserContext | null = null;
        try {
            const browser = await this.getBrowser();
            ...
            await page.route('**/*', (route) => {
                const type = route.request().resourceType();
                if (['media', 'font', 'stylesheet'].includes(type) || this.isBlockedDomain(route.request().url())) {
                    route.abort();
                } else {
                    route.continue();
                }
            ...
            const result = await page.evaluate(() => {
                // @ts-ignore
                if (typeof Readability === 'undefined') return { error: 'Readability not found' };
                // @ts-ignore
            ...
            if (!result || (result as any).error) {
                return { title: '', byline: null, content: '', textContent: '', siteName: null, status: 'failed', error: (result as any)?.error || 'Parse failed' };
            }
    ...
    private async handleWafChallenge(page: Page): Promise<boolean> {
        try {
            const challengeInfo = await page.evaluate(() => {
                ...
                return {
                    isSucuri: html.includes('sucuri.net') || bodyText.includes('Website Firewall'),
                    isCloudflare: html.includes('cf-browser-verification') || bodyText.includes('Checking your browser'),
                    isChallenge: html.includes('sucuri') || html.includes('cf-browser-verification') || bodyText.includes('Verify you are human'),
                };
            });
