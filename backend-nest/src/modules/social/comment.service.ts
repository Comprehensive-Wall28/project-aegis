...
export class CommentService {
    ...
    async getComments(
        ...
        cursorId?: string,
    ): Promise<{ comments: LinkComment[]; totalCount: number; hasMore: boolean }> {
        ...
        const { comments, totalCount } = await this.linkCommentRepository.findByLinkIdWithPagination(
            linkId,
            limit,
            beforeCursor
        );
    ...
    async postComment(
        ...
        const comment = await this.linkCommentRepository.create(linkId, userId, data.content);
    ...
    async deleteComment(
        ...
        const { roomId, room } = await this.linkAccessHelper.verifyLinkAccess(comment.linkId.toString(), userId);
