...
describe('SocialService', () => {
    ...
    describe('createRoom', () => {
        it('should create a room and a default collection', async () => {
            ...
            expect(collectionRepository.create).toHaveBeenCalledWith(expect.objectContaining({ type: 'links' }));
            expect(auditService.logAuditEvent).toHaveBeenCalledWith(userId, 'ROOM_CREATE', 'SUCCESS', undefined, expect.any(Object));
            expect(result).toEqual(mockRoom);
        });

        it('should throw BadRequestException if name is missing', async () => {
            await expect(service.createRoom('id', { name: '', encryptedRoomKey: 'key' }))
    ...
    describe('createInvite', () => {
        it('should generate an invite code for an owner', async () => {
            ...
            expect(socialRepository.updateInviteCode).toHaveBeenCalledWith(roomId, expect.any(String));
        });
        ...
        it('should throw ForbiddenException if user is only a member', async () => {
            ...
            await expect(service.createInvite(userId, roomId))
    ...
    describe('joinRoom', () => {
        ...
        it('should throw BadRequestException if already a member', async () => {
            ...
            await expect(service.joinRoom(userId, 'code', 'key'))
                .rejects.toThrow(BadRequestException);
        });
    });
});
