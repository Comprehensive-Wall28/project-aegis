...
describe('ImageProxyService', () => {
  ...
  describe('proxyImage', () => {
    it('should return cached image if exists', async () => {
      ...
      expect(cachedImageRepo.findByUrl).toHaveBeenCalledWith('https://example.com/image.jpg');
      expect(gridFsService.getFileStreamFromBucket).toHaveBeenCalledWith(mockFileId, 'linkImages');
    ...
    it('should throw BadRequestException for invalid protocol', async () => {
      await expect(service.proxyImage('ftp://example.com/image.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException for private IP (127.0.0.1)', async () => {
      ...
      await expect(service.proxyImage('http://127.0.0.1/image.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException for private IP (10.x.x.x)', async () => {
      ...
      await expect(service.proxyImage('http://10.0.0.1/image.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException for private IP (192.168.x.x)', async () => {
      ...
      await expect(service.proxyImage('http://192.168.1.1/image.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException for link-local IP (169.254.x.x)', async () => {
      ...
      await expect(service.proxyImage('http://169.254.1.1/image.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should fetch and cache new image on cache miss', async () => {
      ...
      const result = await service.proxyImage('https://example.com/new-image.png');
    ...
    it('should throw InternalServerErrorException after max retries', async () => {
      ...
      await expect(service.proxyImage('https://example.com/image.jpg')).rejects.toThrow(
