...
describe('ScraperController', () => {
  ...
  describe('scrape', () => {
    it('should call smartScrape with valid URL', async () => {
      ...
      mockScraperService.smartScrape.mockResolvedValue({
        title: 'Test',
    ...
    it('should throw BadRequestException if URL is invalid', async () => {
      await expect(controller.scrape('not-a-url')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('reader', () => {
    it('should call readerScrape with valid URL', async () => {
      const url = 'https://example.com';
      mockScraperService.readerScrape.mockResolvedValue({
        title: 'Test',
        content: 'html',
        status: 'success',
      });
    ...
    it('should throw BadRequestException if URL is invalid', async () => {
      await expect(controller.reader('not-a-url')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
