...
describe('ScraperService', () => {
        ...
        mockEvaluate.mockImplementation((fn) => {
            if (typeof fn === 'function') {
                ...
                if (fnStr.includes('Readability')) {
                    return Promise.resolve({ title: 'Mock Title', content: 'Mock Content' });
                }
            }
    ...
    describe('advancedScrape', () => {
        it('should retry on blocked status', async () => {
            const spy = jest.spyOn(service as any, 'advancedScrapeInternal')
        ...
        it('should return failed status on error', async () => {
            jest.spyOn(service as any, 'advancedScrapeInternal').mockRejectedValue(new Error('Network error'));
    ...
    describe('readerScrape', () => {
        it('should return failed status if parsing fails', async () => {
            mockEvaluate.mockImplementation((fn) => {
                const fnStr = fn.toString();
                if (fnStr.includes('isSucuri')) return Promise.resolve({ isChallenge: false });
                if (fnStr.includes('Readability')) return Promise.resolve({ error: 'Readability error' });
                return Promise.resolve({});
            });
