...
describe('CalendarService', () => {
    ...
    describe('getEvents', () => {
        ...
        it('should fetch events for a user', async () => {
            ...
            expect(mockCalendarRepository.findMany).toHaveBeenCalledWith({ userId: new Types.ObjectId(userId) });
        });
        ...
        it('should apply start and end date filters', async () => {
            ...
            expect(mockCalendarRepository.findMany).toHaveBeenCalledWith(expect.objectContaining({
                startDate: {
                    $gte: new Date(start),
                    $lte: new Date(end),
                },
    ...
    describe('updateEvent', () => {
        ...
        it('should throw NotFound if event does not exist for user', async () => {
            ...
            await expect(service.updateEvent(userId, eventId, {}, mockReq)).rejects.toThrow(NotFoundException);
        });
        ...
        it('should update event and log action', async () => {
            ...
            mockCalendarRepository.updateById.mockImplementation((id, data) => Promise.resolve({ ...existing, ...data }));

            const result = await service.updateEvent(userId, eventId, { encryptedData: 'new_data' } as any, mockReq);
    ...
    describe('deleteEvent', () => {
        it('should throw NotFound if event not found', async () => {
            mockCalendarRepository.findOne.mockResolvedValue(null);
            await expect(service.deleteEvent('u', 'e', mockReq)).rejects.toThrow(NotFoundException);
        });
