...
describe('FoldersService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(SharedFolder.name),
          useValue: mockSharedFolderModel,
        },
  ...
  describe('getFolders', () => {
    ...
    it('should return root folders including shared ones', async () => {
      ...
      mockSharedFolderModel.find.mockReturnValue({
        populate: jest.fn().mockResolvedValue([]),
      });

      const result = await service.getFolders(userId);
      expect(result).toEqual([]);
      expect(mockFolderRepository.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ parentId: null }),
      );
    });

    it('should throw BadRequest if parentId is invalid', async () => {
      await expect(service.getFolders(userId, 'invalid')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw NotFound if parent folder not found', async () => {
      ...
      await expect(
        service.getFolders(userId, new Types.ObjectId().toString()),
  ...
  describe('deleteFolder', () => {
    ...
    it('should throw BadRequest if it has subfolders', async () => {
      ...
      await expect(service.deleteFolder(userId, folderId)).rejects.toThrow(
  ...
  describe('getFolder', () => {
    it('should build path correctly', async () => {
      ...
      const rootFolder = {
        ...
        name: 'Root',
        parentId: null,
        toObject: () => ({ _id: rootId, name: 'Root' }),
      };
      const childFolder = {
        _id: childId,
        name: 'Child',
        parentId: rootId,
        toObject: () => ({ _id: childId, name: 'Child', parentId: rootId }),
      };
  ...
  describe('moveFiles', () => {
    it('should call VaultService.bulkMoveFiles', async () => {
      ...
      expect(mockVaultService.bulkMoveFiles).toHaveBeenCalledWith(
        userId,
        updates,
        folderId,
      );
