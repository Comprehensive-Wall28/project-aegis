...
> {
  constructor(
    ...
    @InjectModel(SharedFolder.name)
    private readonly sharedFolderModel: Model<SharedFolderDocument>,
  ...
  async getFolders(userId: string, parentId?: string): Promise<any[]> {
    ...
    if (normalizedParentId === null) {
      ...
      const sharedFolderEntries = await this.sharedFolderModel
    ...
    if (!hasAccess) {
      hasAccess = await this.checkSharedAccess(
        normalizedParentId,
        userId,
        parentFolder,
      );
    ...
    if (parentId) {
      ...
      if (parent.ownerId.toString() !== userId)
        throw new ForbiddenException(
          'Cannot create subfolder in non-owned folder',
        );
    }
  ...
  async updateFolder(
    ...
    if (updateFolderDto.color !== undefined)
      update.color = updateFolderDto.color;
