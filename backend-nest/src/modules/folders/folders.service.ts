...
export class FoldersService {
  ...
  async getFolders(
    ...
    try {
      ...
      if (normalizedParentId === null) {
        // Root level
        const ownedFolders = await this.folderRepository.findByOwnerAndParent(
          userId,
          null,
        );
      ...
      const parentFolder =
        await this.folderRepository.findById(normalizedParentId);
      if (!parentFolder) {
  ...
  async getFolder(
    ...
    try {
      ...
      const folder = await this.folderRepository.findByIdAndOwner(
        folderId,
        userId,
      );
  ...
  async updateFolder(
    ...
    try {
      ...
      const folder = await this.folderRepository.updateByIdAndOwner(
        folderId,
        userId,
        update,
      );
  ...
  async deleteFolder(userId: string, folderId: string): Promise<void> {
    try {
      ...
      const filesCount = await this.vaultRepository.countFilesByFolder(
        folderId,
        userId,
      );
      if (filesCount > 0) {
      ...
      const subfoldersCount = await this.folderRepository.countSubfolders(
        folderId,
        userId,
      );
      ...
      // Delete the folder
      const deleted = await this.folderRepository.deleteByIdAndOwner(
        folderId,
        userId,
      );
      if (!deleted) {
  ...
  async moveFiles(
    ...
    try {
      ...
      const modifiedCount = await this.vaultRepository.bulkMoveFiles(
        bulkUpdates,
        userId,
      );
