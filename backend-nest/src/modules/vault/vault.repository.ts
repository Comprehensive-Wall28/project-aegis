...
import { VaultFile, VaultFileDocument, FileStatus } from './schemas/vault-file.schema';
...
export class VaultRepository extends BaseRepository<VaultFileDocument> {
    ...
    async findByIdAndOwner(fileId: string, ownerId: string): Promise<VaultFileDocument | null> {
        return this.findOne({
    ...
    async findByIdAndStream(fileId: string, ownerId: string): Promise<VaultFileDocument | null> {
    ...
    async bulkMoveFiles(
        ...
        const operations: any[] = updates.map(update => ({
            updateOne: {
                filter: { _id: new Types.ObjectId(update.fileId), ownerId: new Types.ObjectId(ownerId) },
                update: {
                    $set: {
