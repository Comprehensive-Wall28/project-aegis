...
describe('VaultService', () => {
  ...
  describe('initiateUpload', () => {
    ...
    it('should initiate upload for GridFS', async () => {
      ...
      mockVaultRepository.create.mockResolvedValue({
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(
        service.initiateUpload(userId, uploadDto, mockReq),
      ).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if storage limit exceeded', async () => {
      mockUsersService.findById.mockResolvedValue({
        totalStorageUsed: 5 * 1024 * 1024 * 1024,
      });
      await expect(
        service.initiateUpload(userId, uploadDto, mockReq),
  ...
  describe('uploadChunk', () => {
    ...
    it('should throw NotFoundException if file session not found', async () => {
      ...
      await expect(
        service.uploadChunk(userId, fileId, Buffer.from(''), 0, 0, 0, 0),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should handle Google Drive chunk upload', async () => {
      ...
      expect(mockVaultRepository.completeUpload).toHaveBeenCalledWith(
        fileId,
        'drive_id',
      );
  ...
  describe('deleteFile', () => {
    ...
    it('should delete file and log action', async () => {
      ...
      expect(mockVaultRepository.deleteByIdAndOwner).toHaveBeenCalledWith(
        fileId,
        userId,
      );
      expect(usersService.updateStorageUsage).toHaveBeenCalledWith(
        userId,
        -100,
      );
    });
