...
export class GoogleDriveService {
  ...
  private initializeClient(): drive_v3.Drive {
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        'Missing OAuth2 credentials: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET are required',
      );
    }
  ...
  async exchangeCode(code: string): Promise<any> {
    ...
    if (tokens.refresh_token) {
      this.logger.log(
        'Successfully retrieved Google Refresh Token. Please update your GOOGLE_REFRESH_TOKEN environment variable.',
      );
  ...
  async initiateUpload(
    ...
    if (!initResponse.ok) {
      ...
      throw new Error(
        `Failed to initiate resumable upload: ${initResponse.status} - ${errorText}`,
      );
    }
    ...
    const response = await this.fetchWithRetry(session.sessionUrl, {
      ...
      body:
        chunk instanceof Buffer
          ...
          : (Readable.toWeb(chunk as Readable) as any),
      duplex: 'half',
    } as any);
  ...
  private async fetchWithRetry(
    ...
    try {
      ...
      if ((response.status >= 500 || response.status === 429) && retries > 0) {
        this.logger.warn(
          `Google Drive API request failed with ${response.status}. Retrying...`,
        );
