...
export class VaultController {
  ...
  async uploadChunk(
    ...
    if (!fileId || !range) {
      throw new BadRequestException(
        'Missing fileId query or Content-Range header',
      );
    }
  ...
  @Post('upload/gridfs')
  async uploadGridFs(@CurrentUser() user: any, @Req() req: FastifyRequest) {
    ...
    for await (const part of parts) {
      if (part.type === 'field') {
        ...
        try {
          const id = await this.gridFsService.uploadStream(
            part.file,
            part.filename,
          );
    ...
    if (fileSaved && fileId && gridFsId) {
      await this.vaultService.completeGridFsUpload(
        user._id.toString(),
        fileId,
        new Types.ObjectId(gridFsId),
      );
      return { success: true };
  ...
  async listFiles(
    ...
    return this.vaultService.listFiles(
      user._id.toString(),
      folderId,
      search,
      limit,
  ...
  @Get('files/:id')
  async getFile(@CurrentUser() user: any, @Param('id') id: string) {
  ...
  async downloadFile(
    ...
    const { stream, mimeType, fileName } =
      await this.vaultService.getDownloadStream(user._id.toString(), id);
