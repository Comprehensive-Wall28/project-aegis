...
> {
  ...
  async initiateUpload(
    ...
    if (user.totalStorageUsed + data.fileSize > this.MAX_STORAGE) {
      throw new ForbiddenException(
        'Storage limit exceeded. Delete some files.',
      );
    }

    // Validate folder access if provided
    if (data.folderId) {
      const hasAccess = await this.foldersService.checkAccess(
        userId,
        data.folderId,
      );
      if (!hasAccess) {
    ...
    } else if (data.fileSize > 50 * 1024 * 1024) {
      // Default to Drive for files > 50MB
  ...
  async uploadChunk(
    ...
    if (file.status === FileStatus.PENDING) {
      await this.vaultRepository.updateUploadStatus(
        fileId,
    ...
    if (file.storageProvider === StorageProvider.GOOGLE_DRIVE) {
      ...
      if (result.complete) {
        const driveFileId = await this.googleDriveService.finalizeUpload(
          file.uploadStreamId,
        );
      ...
      throw new BadRequestException(
        'Chunked upload not supported for GridFS in this version. Use standard multipart upload.',
      );
    }
  }
   ...
   */
  async completeGridFsUpload(
    userId: string,
    fileId: string,
    gridFsId: Types.ObjectId,
  ) {
    const file = await this.vaultRepository.findOne({
      _id: fileId,
      ownerId: userId,
    } as any);
  ...
  async getDownloadStream(
    ...
    if (!file) {
      ...
      if (potentialFile && potentialFile.folderId) {
        const hasAccess = await this.foldersService.checkAccess(
          userId,
          potentialFile.folderId.toString(),
        );
        if (hasAccess) {
          file = potentialFile;
        }
    ...
    if (file.storageProvider === StorageProvider.GOOGLE_DRIVE) {
      ...
      stream = await this.googleDriveService.getFileStream(
  ...
  async deleteFile(
    ...
    if (
      ...
      try {
        ...
        this.logger.warn(
          `Failed to delete Drive file ${file.googleDriveFileId}: ${e.message}`,
        );
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to delete GridFS file ${file.gridFsId}: ${e.message}`,
        );
      }
    }
  ...
  async countFiles(userId: string, folderId: string): Promise<number> {
    return this.vaultRepository.count({
      ownerId: new Types.ObjectId(userId),
  ...
  async getStorageStats(
    userId: string,
  ): Promise<{ totalStorageUsed: number; maxStorage: number }> {
    const user = await this.usersService.findById(userId);
