...
import {
  VaultListingRequestDto,
  VaultListingResponseDto,
} from './dto/vault-listing.dto';
...
export class VaultService {
  ...
  async initUpload(
    ...
    try {
      ...
      const { sessionId, sessionUrl } =
        await this.googleDriveService.initiateUpload(
  ...
  async uploadChunk(
    ...
    try {
      ...
      const fileRecord = await this.vaultRepository.findByIdAndStream(
        fileId,
        userId,
      );
      ...
      const { complete, receivedSize } =
        await this.googleDriveService.appendChunk(
      ...
      if (complete) {
        // Finalize the upload
        const googleDriveFileId = await this.googleDriveService.finalizeUpload(
          fileRecord.uploadSessionUrl,
          totalSize,
        );
  ...
  async getUserFiles(
    ...
    try {
      ...
      return await this.vaultRepository.findByOwnerAndFolder(
        userId,
        folderId || null,
        search,
      );
  ...
  async getDownloadStream(
    ...
    try {
      ...
      const fileRecord = await this.vaultRepository.findByIdAndOwner(
        fileId,
        userId,
      );
