...
describe('GpaService', () => {
    ...
    describe('deleteCourse', () => {
        it('should throw NotFound if course not found', async () => {
            ...
            await expect(service.deleteCourse('u', 'c', mockReq)).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('updatePreferences', () => {
        it('should update gpaSystem', async () => {
            ...
            expect(usersService.updateProfile).toHaveBeenCalledWith('u', { gpaSystem: 'GERMAN' });
        });

        it('should throw BadRequest if gpaSystem is invalid', async () => {
            await expect(service.updatePreferences('u', 'INVALID', mockReq)).rejects.toThrow(BadRequestException);
        });
    });
});
