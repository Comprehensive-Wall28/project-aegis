...
describe('GpaModule (Integration)', () => {
    ...
    it('should create a course with encrypted data', async () => {
        const course = await gpaService.createCourse(userId.toHexString(), {
    ...
    it('should list user courses', async () => {
        await gpaService.createCourse(userId.toHexString(), {
            encryptedData: 'c1', encapsulatedKey: 'k', encryptedSymmetricKey: 'sk'
        } as any, {});
        await gpaService.createCourse(userId.toHexString(), {
            encryptedData: 'c2', encapsulatedKey: 'k', encryptedSymmetricKey: 'sk'
        } as any, {});
    ...
    it('should delete a course', async () => {
        const course = await gpaService.createCourse(userId.toHexString(), {
            encryptedData: 'del', encapsulatedKey: 'k', encryptedSymmetricKey: 'sk'
        } as any, {});

        await gpaService.deleteCourse(userId.toHexString(), (course as any)._id.toString(), {});
    ...
    it('should get GPA preferences (mocked)', async () => {
        ...
        expect(mockUsersService.findById).toHaveBeenCalledWith(userId.toHexString());
    });

    it('should update GPA preferences (mocked)', async () => {
        const result = await gpaService.updatePreferences(userId.toHexString(), 'GERMAN', {});
