...
describe('Auth Module Integration Tests (matching legacy backend)', () => {
    ...
    describe('POST /auth/register', () => {
        it('should register a new user WITHOUT returning token (matches legacy)', async () => {
            ...
            expect(response.body).toHaveProperty('pqcPublicKey', testUser.pqcPublicKey);
    ...
    describe('Legacy Hash Migration', () => {
        ...
        it('should migrate v1 hash to v2 on successful login with legacyHash', async () => {
            ...
            const isValid = await argon2.verify(updatedUser.passwordHash, 'newClientHash'.toLowerCase());
    ...
    describe('Response Format Consistency', () => {
        it('should return consistent user response format across all endpoints', async () => {
            ...
            const expectedKeys = ['_id', 'username', 'email', 'pqcPublicKey', 'preferences', 'hasPassword', 'webauthnCredentials'];
            ...
            // Get Me
            const tokenCookie = loginRes.headers['set-cookie'].find((c: string) => c.startsWith('token='));
