...
import {
  LinkComment,
  LinkCommentDocument,
} from '../../../src/modules/social/schemas/link-comment.schema';
import {
  LinkView,
  LinkViewDocument,
} from '../../../src/modules/social/schemas/link-view.schema';
import {
  ReaderAnnotation,
  ReaderAnnotationDocument,
} from '../../../src/modules/social/schemas/reader-annotation.schema';
...
describe('Social (e2e) - DELETE /api/social/rooms/:roomId', () => {
  ...
  beforeAll(async () => {
    ...
    linkCommentRepository = moduleFixture.get<LinkCommentRepository>(
      LinkCommentRepository,
    );
    linkViewRepository =
      moduleFixture.get<LinkViewRepository>(LinkViewRepository);
    readerAnnotationRepository = moduleFixture.get<ReaderAnnotationRepository>(
    ...
    linkCommentModel = moduleFixture.get<Model<LinkCommentDocument>>(
      getModelToken(LinkComment.name, 'primary'),
    );
    linkViewModel = moduleFixture.get<Model<LinkViewDocument>>(
      getModelToken(LinkView.name, 'primary'),
    );
    readerAnnotationModel = moduleFixture.get<Model<ReaderAnnotationDocument>>(
      getModelToken(ReaderAnnotation.name, 'primary'),
    );
  });
