...
describe('Social (e2e) - DELETE /api/social/collections/:collectionId', () => {
  ...
  beforeAll(async () => {
    ...
    const ownerCookies: string[] = [].concat(
      ownerLoginResponse.headers['set-cookie'] as any,
    );
    ...
    const adminCookies: string[] = [].concat(
      adminLoginResponse.headers['set-cookie'] as any,
    );
    ...
    const memberCookies: string[] = [].concat(
      memberLoginResponse.headers['set-cookie'] as any,
    );
  ...
  it('should delete collection as owner', async () => {
    ...
    const deletedCollection = await collectionRepository.findById(
  ...
  it.skip('should delete all links in collection when deleting', async () => {
    ...
    const linksBefore = await linkPostRepository.findByCollections([
      collection._id.toString(),
    ]);
    ...
    const linksAfter = await linkPostRepository.findByCollections([
      collection._id.toString(),
    ]);
