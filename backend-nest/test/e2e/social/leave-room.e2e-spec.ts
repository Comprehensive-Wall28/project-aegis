...
describe('Social (e2e) - POST /api/social/rooms/:roomId/leave', () => {
    ...
    async function createRoomWithMultipleOwners(owner1Id: string, owner2Id: string): Promise<string> {
        const room = await roomRepository.create({
    ...
    it('should return 400 when last owner tries to leave', async () => {
        ...
        const body = JSON.parse(response.payload);
        expect(body.message).toBe('Cannot leave room as the last owner. Delete the room instead.');
    });
