...
describe('Social (e2e) - DELETE /api/social/links/:linkId', () => {
    ...
    beforeAll(async () => {
        ...
        const room = await roomRepository.create({
            ...
            members: [
                { userId: new Types.ObjectId(ownerUserId), role: 'owner', encryptedRoomKey: 'key1' },
                { userId: new Types.ObjectId(creatorUserId), role: 'member', encryptedRoomKey: 'key2' },
                { userId: new Types.ObjectId(otherMemberUserId), role: 'member', encryptedRoomKey: 'key3' }
            ]
        } as any);
    ...
    it('should delete a link as the creator', async () => {
        ...
        expect(JSON.parse(response.payload).message).toBe('Link deleted successfully');
    ...
    it('should return 403 when trying to delete someone else\'s link as a regular member', async () => {
        ...
        expect(JSON.parse(response.payload).message).toBe('Only post creator or room owner can delete links');
    ...
    it('should return 403 when not a member of the room', async () => {
        ...
        expect(response.statusCode).toBe(403);
        expect(JSON.parse(response.payload).message).toBe('Not a member of this room');
    });
