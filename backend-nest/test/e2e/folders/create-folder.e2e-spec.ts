...
describe('Folders (e2e) - POST /api/folders', () => {
    ...
    it('should return 400 when name is missing', async () => {
        ...
        const body = JSON.parse(response.payload);
        expect(body.message).toEqual(expect.arrayContaining([expect.stringContaining('name')]));
    });
    ...
    it('should return 400 when encryptedSessionKey is missing', async () => {
        ...
        expect(response.statusCode).toBe(400);
        const body = JSON.parse(response.payload);
        expect(body.message).toEqual(expect.arrayContaining([expect.stringContaining('encryptedSessionKey')]));
    });
