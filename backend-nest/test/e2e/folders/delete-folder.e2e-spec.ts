...
describe('Folders (e2e) - DELETE /api/folders/:id', () => {
    ...
    beforeAll(async () => {
        ...
        const otherCookies: string[] = [].concat(otherLoginResponse.headers['set-cookie'] as any);
    ...
    beforeEach(async () => {
        ...
        await folderModel.deleteMany({
            ownerId: { $in: [new Types.ObjectId(userId), new Types.ObjectId(otherUserId)] }
        });
    ...
    it('should return 400 when folder has files', async () => {
        ...
        const body = JSON.parse(response.payload);
        expect(body.message).toBe('Cannot delete folder with files. Move or delete files first.');
    ...
    it('should return 400 when folder has subfolders', async () => {
        ...
        expect(response.statusCode).toBe(400);
        const body = JSON.parse(response.payload);
        expect(body.message).toBe('Cannot delete folder with subfolders. Delete subfolders first.');
