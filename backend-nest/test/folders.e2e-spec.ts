...
describe('Folder Domain E2E Tests (Express Backend)', () => {
    ...
    describe('POST /api/folders (Create)', () => {
        it('should create a folder with valid data', async () => {
            const response = await createFolder(agent, {}, accessToken);
        ...
        it('should create a root level folder with null parentId', async () => {
            const response = await createFolder(agent, rootFolder, accessToken);
            expect(response.status).toBe(201);
        ...
        it('should create a folder with custom color', async () => {
            const response = await createFolder(agent, coloredFolder, accessToken);
            expect(response.status).toBe(201);
            expect(response.body._id).toBeDefined();
            expect(typeof response.body._id).toBe('string');
            // Note: The Express backend doesn't support setting color during creation
        ...
        it('should fail if encryptedSessionKey is missing', async () => {
            ...
            expect(response.status).toBe(400);
            expect(response.body.message).toContain('encryptedSessionKey must be a string');
        });
        ...
        it('should trim whitespace from folder name', async () => {
            const response = await createFolder(agent, {
                name: '  Trimmed Folder  ',
    ...
    describe('GET /api/folders (List)', () => {
        beforeAll(async () => {
            ...
            for (const folderData of multipleFolders) {
                await createFolder(agent, folderData, accessToken);
            }
        });

        it('should return all root folders for the user', async () => {
            const response = await getFolders(agent, { parentId: null }, accessToken);
        ...
        it('should return 200 with empty array when no folders exist', async () => {
            ...
            const response = await getFolders(auth.agent, { parentId: null }, auth.accessToken);
            expect(response.status).toBe(200);
            expect(Array.isArray(response.body)).toBe(true);
        ...
        it('should filter folders by parentId', async () => {
            ...
            const parentRes = await createFolder(agent, parentFolder, accessToken);
            const parentId = parentRes.body._id;

            // Create child folders under the parent
            const child1Res = await createFolder(agent, {
                ...childFolder,
                parentId,
                name: 'Child 1'
            }, accessToken);

            const child2Res = await createFolder(agent, {
                ...childFolder,
                parentId,
                name: 'Child 2'
            }, accessToken);
            ...
            const response = await getFolders(agent, { parentId }, accessToken);
            expect(response.status).toBe(200);
            expect(Array.isArray(response.body)).toBe(true);
        ...
        it('should return 400 for invalid parentId format', async () => {
            const response = await getFolders(agent, { parentId: 'invalid-id' }, accessToken);
            expect(response.status).toBe(400);
        });
    });
    ...
    describe('GET /api/folders/:id (Get Single)', () => {
        ...
        beforeAll(async () => {
            const res = await createFolder(agent, validFolderData, accessToken);
            folderId = res.body._id;
        });

        it('should return a single folder by ID', async () => {
            const response = await getFolder(agent, folderId, accessToken);
            expect(response.status).toBe(200);
        ...
        it('should return folder with path for nested folder', async () => {
            ...
            const childRes = await createFolder(agent, {
                name: 'Child',
                parentId: parentId,
            ...
            const response = await getFolder(agent, childId, accessToken);
            expect(response.status).toBe(200);
    ...
    describe('PUT /api/folders/:id (Update)', () => {
        ...
        it('should update folder name', async () => {
            ...
            const response = await updateFolder(agent, folderId, { name: newName }, accessToken);
            expect(response.status).toBe(200);
        ...
        it('should update folder color', async () => {
            ...
            const response = await updateFolder(agent, folderId, { color: newColor }, accessToken);
            expect(response.status).toBe(200);
            expect(response.body.color).toBe(newColor);
        });

        it('should update multiple fields simultaneously', async () => {
            const response = await updateFolder(agent, folderId, {
                name: 'Multi Updated',
                color: '#00ff00'
            }, accessToken);
            expect(response.status).toBe(200);
        ...
        it('should return 404 for non-existent folder', async () => {
            const fakeId = '507f1f77bcf86cd799439011';
            const response = await updateFolder(agent, fakeId, { name: 'New Name' }, accessToken);
            expect(response.status).toBe(404);
        });

        it('should return 400 when no valid fields provided', async () => {
            const response = await updateFolder(agent, folderId, {}, accessToken);
            expect(response.status).toBe(400);
            expect(response.body.message).toContain('No valid fields');
        });

        it('should return 400 for invalid folder ID format', async () => {
            const response = await updateFolder(agent, 'invalid-id', { name: 'Test' }, accessToken);
        ...
        it('should allow setting color to null', async () => {
            // First set a color
            await updateFolder(agent, folderId, { color: '#123456' }, accessToken);

            // Then try to remove it by setting to null
            const response = await updateFolder(agent, folderId, { color: null }, accessToken);
            expect(response.status).toBe(200);
            // In NestJS, null is correctly handled to clear the field (if that's the logic)
            // or at least it doesn't stay as the old value if the update was successful.
            expect(response.body.color).toBe(null);
        });
    });
    ...
    describe('DELETE /api/folders/:id (Delete)', () => {
        it('should delete an empty folder', async () => {
            ...
            const res = await createFolder(agent, {
            ...
            // Delete the folder
            const deleteRes = await deleteFolder(agent, folderId, accessToken);
            expect(deleteRes.status).toBe(200);
            expect(deleteRes.body.message).toContain('deleted');

            // Verify it's gone
            const getRes = await getFolder(agent, folderId, accessToken);
            expect(getRes.status).toBe(404);
        });

        it('should return 404 when deleting non-existent folder', async () => {
            const fakeId = '507f1f77bcf86cd799439011';
            const response = await deleteFolder(agent, fakeId, accessToken);
            expect(response.status).toBe(404);
        });
        ...
        it('should return 400 when deleting folder with subfolders', async () => {
            ...
            const response = await deleteFolder(agent, parentId, accessToken);
            expect(response.status).toBe(400);
            expect(response.body.message).toContain('subfolders');
        });

        it('should return 400 for invalid folder ID format', async () => {
            const response = await deleteFolder(agent, 'invalid-id', accessToken);
    ...
    describe('PUT /api/folders/move-files (Move Files)', () => {
        it('should return 400 when no updates provided', async () => {
            const response = await moveFiles(agent, [], null, accessToken);
            expect(response.status).toBe(400);
        ...
        it('should accept move to root (null folderId)', async () => {
            ...
            const response = await moveFiles(agent, updates, null, accessToken);
    ...
    describe('Folder Hierarchy', () => {
        it('should create nested folder structure', async () => {
            // Create grandparent
            const gpRes = await createFolder(agent, {
            ...
            // Create parent
            const pRes = await createFolder(agent, {
            ...
            // Create child
            const cRes = await createFolder(agent, {
            ...
            // Create grandchild
            const gcRes = await createFolder(agent, {
        ...
        it('should build correct path for deeply nested folder', async () => {
            ...
            const level1 = await createFolder(agent, {
                name: 'Level 1',
                encryptedSessionKey: 'l1_key'
            }, accessToken);
            const id1 = level1.body._id;

            const level2 = await createFolder(agent, {
                name: 'Level 2',
                parentId: id1,
            ...
            const level3 = await createFolder(agent, {
                name: 'Level 3',
                parentId: id2,
            ...
            const response = await getFolder(agent, id3, accessToken);
            expect(response.status).toBe(200);
        ...
        it('should list only direct children of a folder', async () => {
            ...
            const childRes = await getFolders(agent, { parentId }, accessToken);
    ...
    describe('ObjectID Validation', () => {
        it('should reject invalid ObjectID format in URL', async () => {
            const response = await getFolder(agent, 'invalid-object-id', accessToken);
            expect(response.status).toBe(400);
        });

        it('should reject 12-character string (old ObjectID format)', async () => {
            const response = await getFolder(agent, 'abcdefghijkl', accessToken);
            expect(response.status).toBe(400);
        });
        ...
        it('should reject empty string as folder ID', async () => {
            ...
            // Empty ID hits the list endpoint instead or returns 404/400
            expect([200, 400, 404]).toContain(response.status);
        });

        it('should accept valid 24-character hex ObjectID', async () => {
            const validId = '507f1f77bcf86cd799439011';
            const response = await getFolder(agent, validId, accessToken);
    ...
    describe('Unauthorized Access', () => {
        it('should return 401 when creating folder without token', async () => {
            const response = await request(APP_URL).post('/api/folders').send(validFolderData);
            expect(response.status).toBe(401);
        });

        it('should return 401 when listing folders without token', async () => {
            const response = await request(APP_URL).get('/api/folders');
            expect(response.status).toBe(401);
        });

        it('should return 401 when getting single folder without token', async () => {
            const response = await request(APP_URL).get('/api/folders/507f1f77bcf86cd799439011');
            expect(response.status).toBe(401);
        });
        ...
        it('should return 401 when deleting folder without token', async () => {
            const response = await request(APP_URL).delete('/api/folders/507f1f77bcf86cd799439011');
            expect(response.status).toBe(401);
        });
    ...
    describe('Edge Cases and Boundary Tests', () => {
        ...
        it('should handle rapid sequential folder creation', async () => {
            const promises = [];
            for (let i = 0; i < 5; i++) {
                promises.push(createFolder(agent, {
                    name: `Rapid Folder ${i}`,
        ...
        it('should maintain folder ownership after updates', async () => {
            ...
            // Update the folder
            const updateRes = await updateFolder(agent, folderId, {
