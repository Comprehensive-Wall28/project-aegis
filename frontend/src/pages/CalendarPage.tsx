...
export function CalendarPage() {
    ...
    const dialogOpen = !!urlEditId || !!urlNewDate;
    const previewOpen = !!urlEventId && !dialogOpen;

    // Unified type for selected event state (either a full event or a new template)
    type CalendarEventState = DecryptedCalendarEvent | { start: string; end: string; id?: undefined; _id?: undefined; title?: string; description?: string; location?: string; color?: string; startDate?: string; endDate?: string; isAllDay?: boolean };

    // Derive intentEvent directly from state/URL
    const intentEvent = useMemo<CalendarEventState | null>(() => {
        if (urlEditId) {
            const event = events.find(e => e._id === urlEditId);
            if (event) return { ...event, id: event._id, start: event.startDate, end: event.endDate, allDay: event.isAllDay };
        } else if (urlNewDate) {
            return { start: urlNewDate, end: urlNewDate };
        } else if (urlEventId) {
            const event = events.find(e => e._id === urlEventId);
            if (event) return { ...event, id: event._id, start: event.startDate, end: event.endDate, allDay: event.isAllDay };
        }
        return null;
    }, [urlEditId, urlNewDate, urlEventId, events]);

    // PRESERVE state during closing transition to prevent "flicker"
    const [stickyEvent, setStickyEvent] = useState<CalendarEventState | null>(null);
    if (intentEvent && intentEvent !== stickyEvent) {
        setStickyEvent(intentEvent);
    }

    const selectedEvent = intentEvent || stickyEvent;
